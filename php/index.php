...
<html lang="fr">
	...
	<body>
		<?php
				...
				$req = $baseDeDonne->prepare("SELECT url_photo_temoignage, description_photo_temoignage FROM photo_temoignage" );
				$req->execute();
				//On met les photo dans une variable images
				$images = $req->fetchAll();
				//Requete qui compte le nombre de photo dans la base de donné
				$reqCnt = $baseDeDonne->prepare("SELECT COUNT(*) FROM photo_temoignage");
				$reqCnt->execute();
